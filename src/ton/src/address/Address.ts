...
export class Address {
  ...
  static parseRaw(source: string) {
    const workChain = parseInt(source.split(':')[0])
    const hash = Buffer.from(source.split(':')[1], 'hex')
  ...
  toFriendlyBuffer = (args?: { bounceable?: boolean; testOnly?: boolean }) => {
    const testOnly = args && args.testOnly !== undefined ? args.testOnly : false
    const bounceable = args && args.bounceable !== undefined ? args.bounceable : true
  ...
  toFriendly = (args?: { urlSafe?: boolean; bounceable?: boolean; testOnly?: boolean }) => {
    ...
    const buffer = this.toFriendlyBuffer(args)
    if (urlSafe) {
