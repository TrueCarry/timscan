...
export class TonClient {
  ...
  async callGetMethodWithError(
    ...
    name: string,
    params: any[] = []
  ): Promise<{ gas_used: number; stack: any[]; exit_code: number }> {
    const res = await this.#api.callGetMethod(address, name, params)
  ...
  async getTransaction(address: Address, lt: string, hash: string) {
    const res = await this.#api.getTransaction(address, lt, hash)
    if (res) {
  ...
  async getMasterchainInfo() {
    const r = await this.#api.getMasterchainInfo()
    return {
  ...
  async getWorkchainShards(seqno: number) {
    const r = await this.#api.getShards(seqno)
    return r.map((m) => ({
  ...
  async getShardTransactions(workchain: number, seqno: number, shard: string) {
    const tx = await this.#api.getBlockTransactions(workchain, seqno, shard)
    if (tx.incomplete) {
  ...
  async getContractState(address: Address) {
    ...
    const balance = new BN(info.balance)
  ...
  async createNewWallet(args: {
    ...
    const mnemonic = await mnemonicNew(24, args.password)
    const key = await mnemonicToWalletKey(mnemonic, args.password)
    const kind = args.type || 'org.ton.wallets.v3'
    const wallet = Wallet.openByType(this, args.workchain, key.secretKey, kind)
    return {
      mnemonic,
      key,
      wallet,
