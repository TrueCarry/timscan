...
class TypedCache<K, V> {
  ...
  async get(key: K) {
    const ex = await this.cache.get(this.namespace, this.keyEncoder(key))
    if (ex) {
      const decoded = this.codec.decode(JSON.parse(ex))
      if (isRight(decoded)) {
...
export class HttpApi {
  ...
  private shardTransactionsCache: TypedCache<
    ...
    t.TypeOf<typeof getBlockTransactions>
  >
    ...
    this.shardLoader = new DataLoader(async (src) => {
      return await Promise.all(
        src.map(async (v) => {
          ...
          const loaded = (await this.doCall('shards', { seqno: v }, getShards)).shards
      ...
      async (src) => {
        return await Promise.all(
          src.map(async (v) => {
            ...
            const loaded = await this.doCall(
              'getBlockTransactions',
              { workchain: v.workchain, seqno: v.seqno, shard: v.shard },
              getBlockTransactions
            )
  ...
  async getTransaction(address: Address, lt: string, hash: string) {
    const convHash = Buffer.from(hash, 'base64').toString('hex')
  ...
  async callGetMethod(address: Address, method: string, params: any[]) {
    return await this.doCall(
      'runGetMethod',
      { address: address.toString(), method, stack: params },
      callGetMethod
    )
  }
    ...
    return await this.doCall(
      'estimateFee',
      {
        ...
        init_data: args.initData
          ? (await args.initData.toBoc({ idx: false })).toString('base64')
          : '',
        init_code: args.initCode
          ? (await args.initCode.toBoc({ idx: false })).toString('base64')
          : '',
