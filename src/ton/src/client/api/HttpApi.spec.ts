...
describe('HttpApi', () => {
  it('should get balance', async () => {
    ...
    const res = await api.getAddressInformation(
      Address.parseFriendly('EQDR4neQzqkfEz0oR3hXBcJph64d5NddP8H8wfN0thQIAqDH').address
    )
  ...
  it('should get seqno', async () => {
    const api = new HttpApi('https://testnet.toncenter.com/api/v2/jsonRPC', new InMemoryCache())
    const res = await api.callGetMethod(
      Address.parseFriendly('EQDR4neQzqkfEz0oR3hXBcJph64d5NddP8H8wfN0thQIAqDH').address,
      'seqno',
      []
    )
  ...
  it('should get transactions', async () => {
    ...
    await api.getTransactions(
      Address.parseFriendly('kf91o4NNTryJ-Cw3sDGt9OTiafmETdVFUMvylQdFPoOxIsLm').address,
      { limit: 10 }
    )
    await api.getTransactions(
      Address.parseFriendly('Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF').address,
      { limit: 100 }
    )
  })
  ...
  it('should support paging', async () => {
    const api = new HttpApi('https://mainnet.tonhubapi.com/jsonRPC', new InMemoryCache())
    const tx1 = await api.getTransactions(
      Address.parseFriendly('Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF').address,
      { limit: 10 }
    )
    const tx2 = await api.getTransactions(
      Address.parseFriendly('Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF').address,
      {
        limit: 10,
  ...
  it('should support get transaction', async () => {
    ...
    const tx = await api.getTransaction(
      Address.parseFriendly('Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF').address,
  ...
  it('should get masterchain info', async () => {
    const api = new HttpApi('https://mainnet.tonhubapi.com/jsonRPC', new InMemoryCache())
    const mc = await api.getMasterchainInfo()
    const shards = await api.getShards(mc.last.seqno)
  ...
  it('should estimate fee', async () => {
    const api = new HttpApi('https://testnet.toncenter.com/api/v2/jsonRPC', new InMemoryCache())
    const cell = new Cell()
    const fees = await api.estimateFee(
      Address.parse('EQDR4neQzqkfEz0oR3hXBcJph64d5NddP8H8wfN0thQIAqDH'),
      {
        body: cell,
        initCode: null,
        initData: null,
