...
export function computeStorageFees(data: {
  ...
  if (
    ...
    now < storagePrices[0].utime_since.toNumber() ||
    special
  ) {
    return new BN(0)
  }
  ...
  for (let i = 0; i < storagePrices.length && upto < now; i++) {
    const valid_until =
      i < storagePrices.length - 1
        ? Math.min(now, storagePrices[i + 1].utime_since.toNumber())
        : now
    let payment = new BN(0)
    if (upto < valid_until) {
      const delta = valid_until - upto
      payment = payment.add(
        new BN(storageStat.cells).mul(
          masterchain ? storagePrices[i].mc_cell_price_ps : storagePrices[i].cell_price_ps
        )
      )
      payment = payment.add(
        new BN(storageStat.bits).mul(
          masterchain ? storagePrices[i].mc_bit_price_ps : storagePrices[i].bit_price_ps
        )
      )
...
export function computeFwdFees(msgPrices: MsgPrices, cells: BN, bits: BN) {
  return msgPrices.lumpPrice.add(
    shr16ceil(msgPrices.bitPrice.mul(bits).add(msgPrices.cellPrice.mul(cells)))
  )
}
...
export function computeExternalMessageFees(msgPrices: MsgPrices, cell: Cell) {
  // Collect stats
  const storageStats = collectCellStats(cell)
...
export function computeMessageForwardFees(msgPrices: MsgPrices, cell: Cell) {
  const msg = parseMessage(cell.beginParse())
  const storageStats: { bits: number; cells: number } = { bits: 0, cells: 0 }

  // Init
  if (msg.init) {
    const c = collectCellStats(msg.body)
  ...
  // Body
  const bc = collectCellStats(msg.body)
  ...
  const fees = computeFwdFees(msgPrices, new BN(storageStats.cells), new BN(storageStats.bits))
  const res = fees.mul(msgPrices.firstFrac).shrn(16)
  const remaining = fees.sub(res)
...
function shr16ceil(src: BN) {
  const rem = src.mod(new BN(65536))
  let res = src.shrn(16)
  if (!rem.eqn(0)) {
    res = res.addn(1)
  }
