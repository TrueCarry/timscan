...
export function parseCurrencyCollection(slice: Slice): RawCurrencyCollection {
  ...
  if (slice.readBit()) {
    const dc = slice.readCell()
    if (!dc.isExotic) {
      const pd = parseDict(dc.beginParse(), 32, (s) => s.readVarUIntNumber(5))
...
export function parseCommonMsgInfo(slice: Slice): RawCommonMessageInfo {
  if (!slice.readBit()) {
    ...
    const src = slice.readAddress()
    const dest = slice.readAddress()
    const importFee = slice.readCoins()
    return {
      type: 'external-in',
      src,
      dest,
...
export function parseCreditPhase(slice: Slice): RawCreditPhase {
  const dueFeesColelcted = slice.readBit() ? slice.readCoins() : null
...
export function parseComputePhase(slice: Slice): RawComputePhase {
  ...
  const gasFees = slice.readCoins()

  const vmState = slice.readRef()
  const gasUsed = vmState.readVarUInt(3)
  const gasLimit = vmState.readVarUInt(3)
  const gasCredit = vmState.readBit() ? vmState.readVarUInt(2) : null
  const mode = vmState.readUintNumber(8)
  const exitCode = vmState.readUintNumber(32)
  const exitArg = vmState.readBit() ? vmState.readUintNumber(32) : null // TODO: change to int
  const vmSteps = vmState.readUintNumber(32)
  const vmInitStateHash = vmState.readBuffer(32)
  const vmFinalStateHash = vmState.readBuffer(32)
...
export function parseTransactionDescription(slice: Slice): RawTransactionDescription {
  ...
  if (type === 0x00) {
    ...
    const computePhase: RawComputePhase = parseComputePhase(slice)
  ...
  if (type === 0x01) {
    const storagePhase: RawStoragePhase = parseStoragePhase(slice)
    return {
      type: 'storage',
      storagePhase,
    }
...
export function parseTransaction(workchain: number, slice: Slice): RawTransaction {
  ...
  const hasInMessage = messages.readBit()
  const hasOutMessages = messages.readBit()
  let inMessage: RawMessage | null = null
  if (hasInMessage) {
    inMessage = parseMessage(messages.readRef())
  }
  const outMessages: RawMessage[] = []
  if (hasOutMessages) {
    const dict = messages.readDict(15, (slice) => parseMessage(slice.readRef()))
    for (const msg of Array.from(dict.values())) {
      outMessages.push(msg)
    }
  }

  // Currency collections
  const fees = parseCurrencyCollection(slice)

  // Hash update
  const update = parseHashUpdate(slice.readRef())

  // Description
  const description = parseTransactionDescription(slice.readRef())
...
export function parseShardIdent(cs: Slice) {
  ...
  const shardPrefixBits = cs.readUintNumber(6)
  const workchainId = cs.readIntNumber(32)
  const shardPrefix = cs.readUint(64)
  return {
    shardPrefixBits,
    workchainId,
    shardPrefix,
...
export function parseShardAccounts(cs: Slice): Map<string, RawShardAccountRef> {
  ...
  return parseDict(cs.readRef(), 256, (cs2) => {
    const depthBalanceInfo = parseDepthBalanceInfo(cs2)
    const shardAccount = parseShardAccount(cs2)
    return {
      depthBalanceInfo,
      shardAccount,
    }
...
export function parseMasterchainStateExtra(cs: Slice): RawMasterChainStateExtra {
  ...
  // Read config
  const configAddress = new Address(-1, cs.readBuffer(32))
  const config = cs.readCell()
...
export function parseShardStateUnsplit(cs: Slice): RawShardStateUnsplit {
  ...
  const globalId = cs.readIntNumber(32)
  const shardId = parseShardIdent(cs)
  const seqno = cs.readUintNumber(32)
  const vertSeqNo = cs.readUintNumber(32)
  const genUtime = cs.readUintNumber(32)
  const genLt = cs.readUint(64)
  const minRefSeqno = cs.readUintNumber(32)

  // Skip OutMsgQueueInfo: usually exotic
  cs.readCell()

  const beforeSplit = cs.readBit()

  // Parse accounts
  let accounts: Map<string, RawShardAccountRef>
  const accountsCell = cs.readCell()
  ...
  // Parse extras
  const mcStateExtra = cs.readBit()
  let extras: RawMasterChainStateExtra | null = null
  if (mcStateExtra) {
    const cell = cs.readCell()
    if (!cell.isExotic) {
