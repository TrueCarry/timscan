...
export function hashCell(cell: Cell): Buffer {
  return inCache(cell, (cache) => {
    ...
    const r = sha256_sync(getRepr(cell))
    cache.hash = r
    return r
  })
}
...
export function deserializeCellData(cellData: Buffer, referenceIndexSize: number) {
  ...
  // Build Cell
  const bits = BitString.alloc(1023)
  const refs: number[] = []
...
export function deserializeBoc(serializedBoc: Buffer) {
  ...
  for (const ri of header.root_list) {
...
export function serializeToBoc(
  cell: Cell,
  has_idx = true,
  hash_crc32 = true,
  has_cache_bits = false,
  ...
  return inCache(cell, () => {
    ...
    for (const cell_info of allCells) {
    ...
    if (has_idx) {
      allCells.forEach((cell_data, index) =>
