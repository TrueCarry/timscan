...
class ADNLClient extends EventEmitter {
  ...
  constructor(
    ...
    this.cipher = new aesjs.ModeOfOperation.ctr(
      this.params.txKey,
      new aesjs.Counter(this.params.txNonce)
    )
    this.decipher = new aesjs.ModeOfOperation.ctr(
      this.params.rxKey,
      new aesjs.Counter(this.params.rxNonce)
    )
  ...
  private tryProcess(chunk: Buffer) {
    ...
    const canParse = ADNLPacket.containsFullPacket(this.inBuff)
