...
async function addLc() {
  ...
  // class TempWait {
    ...
    // getMasterchainInfo () {
    //   if (this.lc) {
    //     return this.lc.getMasterchainInfo()
    //   }
    ...
    // getAccountState (...args: unknown[]) {
    //   if (this.lc) {
    //     return this.lc.getAccountState(...args)
    //   }
    ...
    const endWait = () => {
      for (const item of queue) {
        ...
        if (lc[item.method]) {
          lc[item.method](...item.args).then(item.resolve).catch(item.reject)
        }
      }
    }


  // const waiter = new TempWait()
