...
import noop from 'knex/lib/util/noop'
import { isString } from 'knex/lib/util/is'
import { wrapString, columnize as columnize_ } from 'knex/lib/formatter/wrappingFormatter'
...
class QueryCompiler_SQLite3 extends QueryCompiler {
  ...
  insert() {
    ...
    while (++i < insertData.values.length) {
      ...
      while (++i2 < insertData.columns.length) {
        block.push(
          this.client.alias(
            this.client.parameter(current[i2], this.builder, this.bindingsHolder),
  ...
  _merge(updates, columns, insert) {
    ...
    if (updates && Array.isArray(updates)) {
      sql += updates
        .map((column) =>
          wrapString(column.split('.').pop(), this.formatter.builder, this.client, this.formatter)
        )
      ...
      sql += insertData.columns
        .map((column) => wrapString(column.split('.').pop(), this.builder, this.client))
        .map((column) => `${column} = excluded.${column}`)
        .join(', ')
  ...
  jsonRemove(params) {
    const jsonCol = `json_remove(${columnize_(
      ...
      this.client,
      this.bindingsHolder
    )},${this.client.parameter(params.path, this.builder, this.bindingsHolder)})`
