...
import { nanonum } from 'knex/lib/util/nanoid'
import {
  copyData,
  dropOriginal,
  renameTable,
  ...
  setForeignCheck,
  executeForeignCheck,
} from './internal/sqlite-ddl-operations'
import { parseCreateTable, parseCreateIndex } from './internal/parser'
import { compileCreateTable, compileCreateIndex } from './internal/compiler'
import { isEqualId, includesId } from './internal/utils'
    ...
    parsedTable.constraints = parsedTable.constraints.filter((constraint) => {
      if (constraint.type === 'PRIMARY KEY' || constraint.type === 'UNIQUE') {
        return constraint.columns.every(
          (constraintColumn) =>
            constraintColumn.expression || !includesId(columns, constraintColumn.name)
        )
