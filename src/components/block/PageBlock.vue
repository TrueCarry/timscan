    ...
    <section>
      <div class="card">
        ...
        <div v-show="boringFieldsVisible">
          <div v-for="field in boringFields" class="card-row card-row--wide">
        ...
        <div v-if="!boringFieldsVisible" class="card-row card-row--wide">
          <div class="card-row__name">
            <div class="block-boring-expand" @click="boringFieldsVisible = true">
      ...
      <div class="block-nav">
        <div v-show="hasPrevBlocks" class="block-nav__vcontainer">
          <router-link
            ...
            :key="prev.file_hash"
            class="block-nav__pill block-nav__pill--prev"
            :to="{
              name: 'block',
              params: {
        ...
        <div v-if="isMasterChain" class="block-nav__hcontainer">
          <router-link
            class="block-nav__pill block-nav__pill--next"
            :to="{ name: 'block', params: nextBlockLinkParams }"
          >
            {{ $t('block.nav.next') }}
            <svg v-pre xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          <span class="block-nav__pill block-nav__pill--next" @click="goToLatestBlock">
            {{ $t('block.nav.latest') }}
            <svg v-pre xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          <table class="tx-table">
            <thead>
              <tr>
                ...
                <th width="110">
                  <div
                    class="tx-table__cell tx-table__cell--align-center"
                    v-text="$t('tx.workchain')"
                  />
                </th>
            ...
            <tbody v-show="shards === undefined">
              <shard-skeleton v-for="i in 5" :key="`shard_skeleton_${i}`" />
            </tbody>
        ...
        <div v-show="!txHistoryEmpty" class="tx-history-wrap">
          <table class="tx-table">
            ...
            <tbody v-show="txHistoryLoading">
              <tx-skeleton v-for="i in 5" :key="`tx_skeleton_${i}`" />
            </tbody>

            <tx-row v-for="(tx, seqno) in transactions" v-bind="tx" :key="tx.hash" :seqno="seqno" />
          </table>
        </div>
      </div>
    </section>
...
<script>
import { getBlockTransactions, getShards, getLastBlock } from '~/api'
...
export default {
  ...
  computed: {
    ...
    prevBlockLinkParams() {
      return { ...this.$props, seqno: parseInt(this.seqno) - 1 }
    },
